// client/src/components/steps/ProjectsForm.jsx
...
export default function ProjectsForm() {
    ...
    // Live State Subscription: Keeps Zustand Store synchronized
    useEffect(() => {
        const subscription = watch((data) => {
            if (data.projects) {
                const formatted = data.projects.map(proj => ({
                    ...proj,
    ...
    // AI Enhancement Option (🚀 ZERO DB CALL: Only updates state form inputs)
    const handleAIProjectDesc = async (index) => {
        ...
        if (!projectName || !rawDesc) {
            alert("Please fill Project Name and Description first.");
            return;
        }
        ...
        try {
            ...
            if (resData.success) {
                ...
                // Update local Zustand Store only
                const updatedProjectsList = watchedProjects.map((p, i) =>
                    i === index ? { ...p, description: resData.description } : p
                );
                updateResumeData('projects', updatedProjectsList);
            } else {
                alert(resData.message || "AI enhancement failed");
            }
        } catch (error) {
    ...
    // Form Submission (🚀 ZERO DB CALL: Directly goes to Next Step)
    const onSubmit = (data) => {
        try {
    ...
    return (
        <form onSubmit={handleSubmit(onSubmit)}>
            <div className="mb-2 d-flex justify-content-between align-items-center">
                <div>
                    <h4 className="fw-bold mb-0 text-white glow-title">
                        Projects
                    </h4>
                    <p className="text-white-50 small mb-0" style={{ fontSize: '0.8rem' }}>Showcase your technical projects and implementations.</p>
                </div>
                <button
                    type="button"
            ...
            <div style={{ maxHeight: '410px', overflowY: 'auto', paddingRight: '5px' }}>
                {fields.map((field, index) => (
                    <div key={field.id} className="p-3 mb-3 border border-secondary border-opacity-10 rounded-3 position-relative" style={{ background: 'rgba(255,255,255,0.02)' }}>
                        {fields.length > 1 && (
                            <button
                                type="button"
                        ...
                        <div className="row g-2">
                            <div className="col-12 col-sm-6">
                                <label className="form-label small text-white fw-medium mb-1" style={{ fontSize: '0.75rem' }}>Project Name *</label>
                            ...
                            <div className="col-12 col-sm-6">
                                <label className="form-label small text-white fw-medium mb-1" style={{ fontSize: '0.75rem' }}>Tech Stack (Comma Separated) *</label>
                            ...
                            <div className="col-12 col-sm-6">
                                <label className="form-label small text-white fw-medium mb-1" style={{ fontSize: '0.75rem' }}>GitHub Repository Link</label>
                            ...
                            <div className="col-12 col-sm-6">
                                <label className="form-label small text-white fw-medium mb-1" style={{ fontSize: '0.75rem' }}>Live Demo Link</label>
                            ...
                            <div className="col-12">
                                <div className="d-flex justify-content-between align-items-center mb-1">
                                    <label className="form-label small text-white fw-medium mb-0" style={{ fontSize: '0.75rem' }}>Project Description *</label>
                                ...
                                <textarea {...register(`projects.${index}.description`, { required: true })} className="form-control glass-input py-1.5" rows="3" placeholder="Write basic actions..." style={{ fontSize: '0.85rem', resize: 'none' }} />
                            </div>
                        </div>
                    </div>
                ))}
